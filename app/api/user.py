...
def update_profile(
    user: UserAuthSchema, current_user: Annotated[UserAuthSchema, Depends(get_current_user)]
):
    try:
        users = repositories.db_users.update(str(current_user.id), user.model_dump(exclude_unset=True))
        print(users)
        user_schema=UserAuthSchema(
